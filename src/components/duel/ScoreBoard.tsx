...
export default function ScoreBoard({ scoreRed, scoreBlue, totalQuestions, currentQuestion }: ScoreBoardProps) {
  return (
    <div className="absolute top-0 left-0 right-0 flex justify-between items-start p-4 pointer-events-none z-10">
      <motion.div
        className="flex flex-col items-center bg-red-50 border border-red-200 rounded-2xl px-6 py-3 min-w-[100px]"
        animate={{ scale: [1, 1.08, 1] }}
        transition={{ duration: 0.3 }}
        key={`red-${scoreRed}`}
      >
        <span className="text-xs font-semibold text-red-400 uppercase tracking-widest">Tim Merah</span>
      ...
      <motion.div
        className="flex flex-col items-center bg-blue-50 border border-blue-200 rounded-2xl px-6 py-3 min-w-[100px]"
        animate={{ scale: [1, 1.08, 1] }}
        transition={{ duration: 0.3 }}
        key={`blue-${scoreBlue}`}
      >
        <span className="text-xs font-semibold text-blue-400 uppercase tracking-widest">Tim Biru</span>
