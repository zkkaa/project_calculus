...
export default function RoundResult({
    ...
    return (
        ...
        >
            ...
            {/* Banner khusus jika semua eliminated */}
            {allEliminated && (
                <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                    transition={{ type: 'spring', stiffness: 300, damping: 20 }}
                    className="text-center bg-red-500/20 border border-red-400/40 rounded-2xl px-8 py-4"
                >
                    <div className="text-5xl mb-2">💥</div>
                    <h2 className="text-3xl font-black text-white">Semua Pemain Gugur!</h2>
                    <p className="text-red-300 mt-1">Tidak ada pemenang di babak ini. Game berakhir.</p>
                </motion.div>
            )}

            {/* ... (sisa komponen sama seperti sebelumnya) */}
            {/* Jawaban benar */}
            <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            {winners.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <p className="text-green-400 text-sm font-bold mb-3">✅ {winners.length} pemain menjawab benar!</p>
            ...
            {eliminated.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <p className="text-red-400 text-sm font-bold mb-3">💀 {eliminated.length} pemain tereliminasi!</p>
                    <div className="flex flex-wrap justify-center gap-2">
                        {eliminated.map(p => (
            ...
            {isAdmin && (
                <motion.button initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: 0.9 }} onClick={onContinue}
                ...
                >
                    {allEliminated ? '💀 Lihat Hasil Akhir' : isLastQuestion ? '🏆 Lihat Hasil Akhir' : `➡️ Soal Berikutnya (${questionNumber}/${totalQuestions})`}
                </motion.button>
            )}
            {!isAdmin && (
